...
ggplot(sparxcc_day_common_samples, aes(x = CorrelationCoefficient, fill = correlation_type)) +
    ...
    labs(title = "Distribution of SparXCC Correlations (Day)",
             x = "Correlation",
             y = "Frequency",
...
ggplot(sparxcc_night_common_samples, aes(x = CorrelationCoefficient, fill = correlation_type)) +
    geom_histogram(binwidth = 0.01, color = "black", alpha = 0.7) +
    scale_fill_manual(values = c("Positive" = "blue", "Negative" = "red")) +
    labs(title = "Distribution of SparXCC Correlations (Night)",
             x = "Correlation",
             y = "Frequency",
             fill = "Correlation Type") +
    theme_minimal()
...
## Clustering the bi-partite networks generated in SparXCC

Creating the bi-partite networks for day and night edge lists:
...
day_node_types <- c(
  setNames(rep(TRUE, length(unique(sparxcc_day_common_samples$OTU))), unique(sparxcc_day_common_samples$OTU)),  # OTU (TRUE)
  setNames(rep(FALSE, length(unique(sparxcc_day_common_samples$Gene))), unique(sparxcc_day_common_samples$Gene))  # Gene (FALSE)
)
night_node_types <- c(
  setNames(rep(TRUE, length(unique(sparxcc_night_common_samples$OTU))), unique(sparxcc_night_common_samples$OTU)),  # OTU (TRUE)
  setNames(rep(FALSE, length(unique(sparxcc_night_common_samples$Gene))), unique(sparxcc_night_common_samples$Gene))  # Gene (FALSE)
)

# Convert correlations to absolute values
sparxcc_day_common_samples$CorrelationCoefficient <- abs(sparxcc_day_common_samples$CorrelationCoefficient)
sparxcc_night_common_samples$CorrelationCoefficient <- abs(sparxcc_night_common_samples$CorrelationCoefficient)

# Creating the graph objects considering absolute values only (magnitude of associations are revelant)
day_bipartite_graph_abs <- graph_from_data_frame(d = sparxcc_day_common_samples, directed = FALSE)
night_bipartite_graph_abs <- graph_from_data_frame(d = sparxcc_night_common_samples, directed = FALSE)

# Assigning the node types
V(day_bipartite_graph_abs)$type <- day_node_types[V(day_bipartite_graph_abs)$name]
E(day_bipartite_graph_abs)$cor <- sparxcc_day_common_samples$CorrelationCoefficient
V(night_bipartite_graph_abs)$type <- night_node_types[V(night_bipartite_graph_abs)$name]
E(night_bipartite_graph_abs)$cor <- sparxcc_night_common_samples$CorrelationCoefficient
```
